...
const FoodCard: React.FC<FoodCardProps> = ({ posting, user, onUpdate }) => {
  ...
  useEffect(() => {
    if (isZoomed || showTracking || showChat) {
      document.body.style.overflow = 'hidden';
    } else {
      document.body.style.overflow = 'unset';
    }
  }, [isZoomed, showTracking, showChat]);
        ...
        <div className="p-5 flex-1 flex flex-col">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="flex flex-col items-end gap-1">
              <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider shrink-0 ${getStatusColor(posting.status)}`}>
                {posting.status}
              </span>
          ...
          <div className="flex flex-col space-y-2 mt-auto">
            ...
            {canVolunteer && (
              <button 
                onClick={() => onUpdate(posting.id, { status: FoodStatus.IN_TRANSIT, volunteerId: user.id, volunteerName: user.name })}
            ...
            {canComplete && (
              <div className="space-y-2">
                ...
                <button 
                  ...
                  className="w-full bg-slate-800 hover:bg-slate-900 text-white font-black py